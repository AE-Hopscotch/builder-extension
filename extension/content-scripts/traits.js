...
const TraitEditor = {
  ...
  init: function () {
    ...
    const traits = [
      ...
      { id: 'edited_at', name: 'Edited At', type: 'text', fields: 1, pattern: '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z$', required: true },
      ...
      { id: 'video_url_path', name: 'Video URL Path', type: 'url', fields: 1 }
    ]
    const html = traits.map(t => {
      ...
      return `<label data-trait-name="${t.id}" ${forIfNeeded}>${t.name}: ${
        new Array(t.fields).fill(`<input type="${t.type}" ${stepIfNeeded} ${patternIfNeeded}>`).join(t.separator)
      }</label>`
    }).join('')
